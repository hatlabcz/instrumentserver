...
class InstrumentClientMainWindow(QtWidgets.QMainWindow):
    ...
    def addWidget(self, widget: QtWidgets.QWidget, name: str, visible: bool = True,
                  position: Qt.DockWidgetArea_Mask = Qt.TopDockWidgetArea):
...
class ServerWidget(QtWidgets.QWidget):
    def __init__(self, client_station:ClientStation, parent=None):
        ...
        # Put label on its own row, then the editor spanning the full width
...
def _param_update_formatter(record, raw_msg):
    _param = re.compile(r'^parameter-update:\s*([A-Za-z0-9_.]+):\s*(.+)\s*$', re.S)
    m = _param.match(raw_msg)
    if not m:
        return None
    name, value = m.groups()
    # escape HTML special chars, but keep raw \n (QTextEdit.append will render them correctly)
    return (
        f"<b>{escape(name)}</b>: "
...
class ClientStationGui(QtWidgets.QMainWindow):
    def __init__(self, station: ClientStation, hide_config:Union[str, dict]=None):
        ...
        self.log_widget.handler.set_transform(_param_update_formatter)
    ...
    def addParameterLoadSaveToolbar(self):
        ...
        browseBtn = QtWidgets.QToolButton(pathWidget)
        browseBtn.setIcon(QtGui.QIcon(":/icons/folder.svg"))
        browseBtn.setText("Browse")
        browseBtn.setToolButtonStyle(QtCore.Qt.ToolButtonTextBesideIcon)
        browseBtn.clicked.connect(self.browseParamPath)
